...
class ShipperApiApplicationTests {
	...
	public void saveTransporterTest() {
		shipper1.setId("transporter:01");
		shipper1.setName("Ram");
		shipper1.setKyc("kyc");
		shipper1.setApproved(false);
		shipper1.setPhoneNo(1234456789);
		shipper2.setId("transporter:02"); 
		shipper2.setName("Seeta");
		shipper2.setKyc("kycSeeta");
		shipper2.setApproved(true);
		shipper2.setPhoneNo(1796546650);
		shipper3.setId("transporter:03");
		shipper3.setName("Laxman");
		...
		assertEquals(tcr, service.addShipper(shipper1));
		when(shipperDao.save(shipper2)).thenReturn(shipper2);
		assertEquals(tcr, service.addShipper(shipper2));
		when(shipperDao.save(shipper3)).thenReturn(shipper3);
		assertEquals(tcr, service.addShipper(shipper3));
	}
	...
	public void updateWithCorrectIdTest() {
		...
		when(shipperDao.findById("shipper:03")).thenReturn(t);
		assertEquals(tur, service.updateShipper("shipper:03", shipper3));
	}
	...
	public void updateWithIncorrectIdTest() {
		...
		when(shipperDao.findById("shipper:01")).thenReturn(t);
		assertEquals(tur, service.updateShipper("shipper:10", shipper1));
	}
	...
	public void updateWithPhoneNoTest() {
		...
		Optional<Shipper> t = Optional.of(shipper1);
		when(shipperDao.findById("shipper:01")).thenReturn(t);
		assertEquals(tur, service.updateShipper("shipper:01", shipper1));
	}
	...
	public void deleteWithCorrectIdTest() {
		...
		when(shipperDao.findById("shipper:02")).thenReturn(t);
		assertEquals(tdr, service.deleteShipper("shipper:02"));
	...
	public void deleteWithIncorrectIdTest() {
		...
		Optional<Shipper> t = Optional.of(shipper1);
		when(shipperDao.findById("shipper:01")).thenReturn(t);
		assertEquals(tdr, service.deleteShipper("transporter:15"));
